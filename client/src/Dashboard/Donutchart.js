...
const Donutchart = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/project", config).then((res => {
            ...
            // setChartData(res.data.countByActive[0].count)
            // setChartData1(res.data.completedProjects[0].count)
            // setChartData2(res.data.overdueProjects[0].count)
            // setChartData3(res.data.canceledProjects[0].count)
