...
const getAllEvents = async (req, res) => {
  ...
  try {
    specEvents = await Event.find({ eventDate: { $gte: startDate, $lte: endDate } });
    console.log("specEvents :", specEvents);
  ...
  return res.status(200).json({ events, allSpecEvents : specEvents });
};
